...
import { useStoreRoleInfo } from "./RoleInfo.store.ts";
...
export namespace useStoreMyUserinfo {
  /**
   * 該当の権限を持っているか確認する
  ...
  export const getRolePower = (
    ...
    for (const index in storeMyUserinfo.RoleLink) {
      ...
      //ロール情報を取得
      const role = useStoreRoleInfo.getterRoleInfo(
        storeMyUserinfo.RoleLink[index].roleId,
      );
