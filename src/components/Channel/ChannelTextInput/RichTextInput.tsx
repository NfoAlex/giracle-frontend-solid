...
export default function RichTextInput(props: {
  ...
  const getParsedJSON = (): IInputSections[] => {
    if (!editorRef) return [];
  ...
  const renderJSONtoHTML = (sections: IInputSections[]): string => {
    let html = "";
    sections.forEach((sec) => {
      if (sec.type === "text") {
        const div = document.createElement("div");
        div.innerText = sec.value;
        html += div.innerHTML;
      } else if (sec.type === "mention") {
        html += `<span class="inline-block bg-primary/20 text-primary px-1.5 py-0.5 rounded font-medium mx-0.5 select-none" contenteditable="false" data-user-id="${sec.lockedUserId}">@${sec.value}</span>`;
  ...
  createEffect(() => {
    const currentJSON = getParsedJSON();
    if (JSON.stringify(props.value) !== JSON.stringify(currentJSON)) {
      if (editorRef) {
        editorRef.innerHTML = renderJSONtoHTML(props.value);
