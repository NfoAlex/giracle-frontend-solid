...
export default function MessageLinkPreview(props: { channelId: string, messageId: string }) {
  const message = createMemo(() => useStoreMessageFetchCache.getMessage(props.channelId, props.messageId));
  ...
  return (
    <Show when={!useStoreMessageFetchCache.getIsDeleted(props.messageId)}>
